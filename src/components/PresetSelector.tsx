    ...
    <div className={`w-full space-y-5 ${isOpen ? 'relative z-50' : ''}`}>
      ...
      <div className="relative">
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-4 bg-slate-50/80 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600/50 rounded-xl hover:border-blue-400 dark:hover:border-blue-500 hover:bg-white dark:hover:bg-slate-700/50 transition-all duration-200 group"
        >
        ...
        {/* Dropdown Panel */}
        {isOpen && (
          <div className="absolute z-[100] w-full mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50 max-h-96 overflow-hidden">
            {/* Search */}
            <div className="p-3 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50 sticky top-0 z-10">
              <div className="relative">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all placeholder:text-slate-400"
                />
              </div>
              {searchQuery && (
            ...
            <div className="overflow-y-auto max-h-72">
              {Object.entries(filteredGroups).map(([category, presets]) => (
                <div key={category}>
                  <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                    {category}
                  </div>
                  {presets.map((preset) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-all ${
                        selectedPreset?.id === preset.id ? 'bg-blue-50 dark:bg-blue-500/10' : ''
                      }`}
                    >
      ...
      {isCustomMode && (
        <div className="p-5 bg-gradient-to-br from-blue-50 via-indigo-50/50 to-violet-50 dark:from-blue-500/10 dark:via-indigo-500/10 dark:to-violet-500/10 rounded-xl border border-blue-200/50 dark:border-blue-500/20">
